...
NOW=$(date -d 'now' +%s%N)
TEMPDIR_1="$NOW.user_temp_1"
TEMPDIR_2="$NOW.user_temp.2"
OUTPUT_1="$NOW.each.user.txt"
OUTPUT_2="$NOW.total.user.txt"
OUTPUT="$RFUNC_user_ratio.txt"
DELIMITER=','
...
select bh_tr, count(distinct(bh_did))
...
echo "total user_num: "$(cat $OUTPUT_2) > $OUTPUT
echo -e "tr\tuser_num\tuser_ration" >> $OUTPUT
